...
export class loginservice {
  ...
  public async getLoginUser(email = undefined, ...others) {
    try {
      let bh = { input: { email: email }, local: { result: undefined } };
      bh = this.__constructDefault(bh);

      bh = await this.sd_6oMTuL4ALMSC1V6F(bh);
  ...
  async sd_6oMTuL4ALMSC1V6F(bh) {
    try {
      bh.local.apiUrl = `http://localhost:24483/api/login?email=${bh.input.email}`;

      bh = await this.sd_vIG9Vy2HLAXjQeZ7(bh);
      //appendnew_next_sd_6oMTuL4ALMSC1V6F
      return bh;
    } catch (e) {
      throw e;
    }
  }
  async sd_vIG9Vy2HLAXjQeZ7(bh) {
    try {
      let requestOptions = {
        url: bh.local.apiUrl,
        method: 'get',
      ...
      //appendnew_next_sd_vIG9Vy2HLAXjQeZ7
      return bh;
    } catch (e) {
      throw e;
    }
